...
document.querySelectorAll('.form-tw').forEach(form => {
  form.addEventListener('submit', function (e) {
    ...
    if (validator.isEmpty(subjectInput.value.trim())) {
      errors.push(lang === 'uk' ? "Будь ласка, введіть тему" : "Please enter a subject");
    }

    const messageInput = form.querySelector('[name="message"]');
    if (validator.isEmpty(messageInput.value.trim())) {
      errors.push(lang === 'uk' ? "Будь ласка, введіть повідомлення" : "Please enter a message");
    }
    ...
    .then(response => {
      if (response.ok) {
        const submitButton = form.querySelector('button[type="submit"]');
        const successText = submitButton.getAttribute(`data-${lang}-success`) || (lang === 'uk' ? "Повідомлення надіслано ✓" : "Message sent ✓");
        submitButton.textContent = successText;
        form.reset();
      } else {
          showAlertByLang(lang, 'error');
...
document.querySelectorAll('.form-footer').forEach(form => {
  form.addEventListener('submit', function (e) {
    ...
    .then(response => {
      if (response.ok) {
        const submitButton = form.querySelector('button[type="submit"]');
        const successText = submitButton.getAttribute(`data-${lang}-success`) || (lang === 'uk' ? "Підписка оформлена ✓" : "Subscribed ✓");
        submitButton.textContent = successText;
        form.reset();
      } else {
          showAlertByLang(lang, 'error');
